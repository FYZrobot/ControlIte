...
if (abs(e1)<=1e-1*pi/180)   %反馈误差条件
    break;
elseif  error(j,1) < (q_pd-qd(failnum,1))           %%%重点设计反馈律
...
% tt=1:n;
% figure(1)%%%角度
% plot(tt*t0,q(1,tt)*180/pi,'k-',tt*t0,q(2,tt)*180/pi,'r-',tt*t0,q(3,tt)*180/pi,'b-',tt*t0,q(4,tt)*180/pi,'g-',...
%     tt*t0,q(5,tt)*180/pi,'c-',tt*t0,q(6,tt)*180/pi,'m-',tt*t0,q(7,tt)*180/pi,'k--','linewidth',2);
% xlabel('时间 ( s )');
% ylabel('关节角度（°）');
% title('关节角度曲线');
...
% grid on;
% figure(2)%%%速度
% plot(tt*t0,vq(1,tt)*180/pi,'k-',tt*t0,vq(2,tt)*180/pi,'r-',tt*t0,vq(3,tt)*180/pi,'b-',tt*t0,vq(4,tt)*180/pi,'g-',tt*t0,vq(5,tt)*180/pi,'c-',...
%     tt*t0,vq(6,tt)*180/pi,'m-',tt*t0,vq(7,tt)*180/pi,'k--','linewidth',2);
% xlabel('时间 ( s )');
% ylabel('关节速度（°/s）');
% title('关节速度曲线');
% legend('关节1','关节2','关节3','关节4','关节5','关节6','关节7');
...
% hold on;
% figure(3)%%%加速度
% plot(tt*t0,aq(1,tt)*180/pi,'k-',tt*t0,aq(2,tt)*180/pi,'r-',tt*t0,aq(3,tt)*180/pi,'b-',tt*t0,aq(4,tt)*180/pi,'g-',tt*t0,aq(5,tt)*180/pi,'c-',...
%     tt*t0,aq(6,tt)*180/pi,'m-',tt*t0,aq(7,tt)*180/pi,'k--','linewidth',2);
% xlabel('时间 ( s )');
% ylabel('关节加速度（°/s^2）');
% title('关节加速度曲线');
% legend('关节1','关节2','关节3','关节4','关节5','关节6','关节7');
...
% hold on;
% figure(4)%%%力矩
% plot(tt*t0,taom(1,tt),'k-',tt*t0,taom(2,tt),'r-',tt*t0,taom(3,tt),'b-',tt*t0,taom(4,tt),'g-',tt*t0,taom(5,tt),'c-',tt*t0,taom(6,tt),'m-',...
%     tt*t0,taom(7,tt),'k--','linewidth',2);
% xlabel('时间 ( s )');
% ylabel('关节力矩 （N.m）');
% title('关节力矩曲线');
% legend('关节1','关节2','关节3','关节4','关节5','关节6','关节7');
